...
//>>> UseNeXT Support
	CServer* newserv = theApp.serverlist->GetServerByAddress(_T("80.239.200.99"), 3000); 
	if(!newserv)
	{
		newserv = new CServer(3000, _T("80.239.200.99"));

		if (!theApp.emuledlg->serverwnd->serverlistctrl.AddServer(newserv, true))
		{
			CServer* update = theApp.serverlist->GetServerByAddress(newserv->GetAddress(), newserv->GetPort());
			if (update)
...
bool CServerList::SaveServermetToFile()
{
	...
	try{
		...
		for (UINT j = 0; j < fservercount; j++)
		{
			...
			if (nextserver->GetPreference(true) != SRV_PR_NORMAL){ //>>> UseNeXT Support
				CTag serverpref(ST_PREFERENCE, nextserver->GetPreference(true)); //>>> UseNeXT Support
