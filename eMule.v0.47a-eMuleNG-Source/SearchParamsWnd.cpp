...
//>>> UseNeXT Support	
	iml.Add(CTempIconLoader(_T("SearchMethod_SERVER"), 16, 16));
	iml.Add(CTempIconLoader(_T("Zoozle"), 16, 16));
//<<< UseNeXT Support
...
void CSearchParamsWnd::InitMethodsCtrl()
{
	...
	VERIFY( m_ctlMethod.AddItem(_T("Usenet"), 4) == SearchTypeUseNeXT ); //>>> UseNeXT Support
	...
	VERIFY( m_ctlMethod.AddItem(_T("Zoozle (Web)"), 5) == SearchTypeZoozle ); //>>> Zoozle	
...
void CSearchParamsWnd::UpdateUnicodeCtrl()
{
	...
	if ((ESearchType)m_ctlMethod.GetCurSel() != SearchTypeFileDonkey 
		&& (ESearchType)m_ctlMethod.GetCurSel() != SearchTypeZoozle) //>>> UseNeXT Support
	{
		CString strExpr;
