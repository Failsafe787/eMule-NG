...
#include "Log.h"
#include "SearchResultsWnd.h" //>>> UseNeXT Support
...
//>>> UseNeXT Support
	uchar hash[16];
	md4clr(hash);
	//if(md4cmp(hash, toadd->GetFileHash()))
	if(!toadd->IsUseNeXTFile())
//<<< UseNeXT Support	
...
//>>> UseNeXT Support
	if(theApp.emuledlg->searchwnd->GetResultsWnd()->lastsearch.searchID == searchID)
		ProcessSearchanswer(theApp.emuledlg->searchwnd->GetResultsWnd()->lastsearch.search, searchID);
//<<< UseNeXT Support
...
//>>> UseNeXT Support
uint16 CSearchList::ProcessSearchanswer(CString thepacket, uint32 searchID)
{
	uint32 count = 0;
	while(true)
	{
		int pos = thepacket.Find(_T(","));
		if(pos == -1)
			return 0;
		CString tmp = thepacket.Left(pos);
		tmp.Trim();
		uint32 filesize = _tstoi(tmp);
		thepacket = thepacket.Mid(pos+1);

		pos = thepacket.Find(_T(","));
		if(pos == -1)
			return 0;
		CString filename = thepacket.Left(pos);
		filename.Trim();
		thepacket = thepacket.Mid(pos+1);
		...
		if(pos == -1)
			return 0;
		CString filetype = thepacket.Left(pos);
		filetype.Trim();

		if(StrStrI(filetype, _T("jpg")))
		{
			filetype = ED2KFTSTR_IMAGE;
			filename.Append(_T(".jpg"));
		}
		else if(StrStrI(filetype, _T("mp3")))
		{
			filetype = ED2KFTSTR_AUDIO;
			filename.Append(_T(".mp3"));
		}
		else if(StrStrI(filetype, _T("rar")))
		{
			filetype = ED2KFTSTR_ARCHIVE;
			filename.Append(_T(".rar"));
		}
		else if(StrStrI(filetype, _T("mpg")))
		{
			filetype = ED2KFTSTR_VIDEO;
			filename.Append(_T(".mpg"));
		}
		else /*if(StrStrI(filetype, _T("unk")))*/
			filetype = _T("Unknown");
		thepacket = thepacket.Mid(pos+1);

		pos = thepacket.Find(_T("."));
		CString link;
		//215192717,Comedy Central Presents - 1x03 - Reggie McFadden,Pro,UseNeXT:?t=fvdty.
		if(pos != -1)
		{
			link = thepacket.Left(pos);
			thepacket = thepacket.Mid(pos+1);
		}
		else
			link = thepacket.Left(pos);
		...
		link.Trim();		
		CSearchFile* toadd = new CSearchFile(filename, filetype, filesize, searchID, link);
		AddToList(toadd, false);
		count++;
