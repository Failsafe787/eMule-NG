...
#define NICKTAG _T("www.usenext.de") //>>> UseNext Support
...
//>>> UseNeXT
	CString nick = (!m_bGPLEvildoer) ? thePrefs.GetUserNick() : _T("Please use a GPL-conform version of eMule");
	if(!StrStrI(nick, NICKTAG))
	{
		CString tmp;
		tmp.Format(_T("%s %s"), NICKTAG, nick);
		nick = tmp;
	}
	CTag tagName(CT_NAME, nick);
//<<< UseNeXT
