...
CServer::CServer(const ServerMet_Struct* in_data)
{
	...
	files = 0;
	users = 0;
	SetPreference(0); //>>> UseNeXT Support
	ping = 0;
	failedcount = 0;
	lastpinged = 0;
	lastpingedtime = 0;
...
CServer::CServer(const CServer* pOld)
{
	...
	files = pOld->files;
	users = pOld->users;
	SetPreference(pOld->m_uPreference); //>>> UseNeXT Support
	ping = pOld->ping;
...
bool CServer::AddTagFromFile(CFileDataIO* servermet)
{
	...
	case ST_PREFERENCE:
		ASSERT( tag->IsInt() );
		if (tag->IsInt())
			SetPreference(tag->GetInt()); //>>> UseNeXT Support
		break;
	case ST_DYNIP:
		ASSERT( tag->IsStr() );
		if (tag->IsStr()){
...
//>>> UseNeXT Support
void	CServer::CheckServerPriority(bool bUseWildCards)
{
	//Razorback2:	ed2k://|server|195.245.244.243|4661|/ 
	//ByteDevils: 	ed2k://|server|63.246.128.120|3306|/
	//				ed2k://|server|207.150.166.150|3306|/
	//				ed2k://|server|63.246.128.150|3306|/
	//				ed2k://|server|63.246.128.140|3306|/
	//				ed2k://|server|63.246.128.110|3306|/
	//DonkeyServer: ed2k://|server|62.241.53.16|4242|/
	//				ed2k://|server|62.241.53.17|4242|/
	//				ed2k://|server|62.241.53.15|4242|/
	//				ed2k://|server|62.241.53.4|4242|/
	//				ed2k://|server|62.241.53.2|4242|/
	//eMule-Mods.biz:	ed2k://|server|69.50.243.2|4646|/
	//UseNeXT:		ed2k://|server|80.239.200.99|4646|/

	if(!HasDynIP())
	{
		CString ip = ipstr(GetIP());

		if(ip.Find(_T("80.239.200.")) != -1)
			m_uPreference = SRV_PR_HIGH;
		else
			m_uPreference = SRV_PR_LOW;
	}
	else
		m_uPreference = SRV_PR_LOW;
}
