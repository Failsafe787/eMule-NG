...
#include "UserMsgs.h"
#include "Log.h"
#include <wininet.h> //>>> UseNeXT Support
...
void CSearchResultsWnd::StartSearch(SSearchParams* pParams)
{
	...
	{
		case SearchTypeUseNeXT: //>>> UseNeXT Support
...
CString	CSearchResultsWnd::CreateWebQuery(SSearchParams* pParams)
{
	...
	{
//>>> UseNeXT Support
		case SearchTypeZoozle: 
			query.Format(_T("http://www.zoozle.net/zoozle.php?&q=%s&s=emule"), EncodeURLQueryParam(pParams->strExpression));
			break;
//<<< UseNeXT Support
...
//>>> UseNeXT Support
			if(sel_file->IsUseNeXTFile())
			{
				if(thePrefs.IsUseNeXTDetected())
					ShellExecute(NULL, NULL, sel_file->GetUseNeXTLink()+_T("&dl"), NULL, NULL, SW_SHOWNORMAL);
				else
					//ShellExecute(NULL, NULL, _T("http://www.usenext.de"), NULL, thePrefs.GetAppDir(), SW_SHOWDEFAULT);
					ShellExecute(NULL, _T("open"), _T("iexplore"), _T("http://www.usenext.de/index.cfm?TD=367992"), thePrefs.GetAppDir(), SW_SHOWDEFAULT);
					//ShellExecute(NULL, _T("open"), _T("iexplore"), _T("http://www.usenext.de"), thePrefs.GetAppDir(), SW_SHOWDEFAULT);
				continue;
			}
//<<< UseNeXT Support
...
//>>> UseNeXT Support
	if(eSearchType == SearchTypeUseNeXT)
	{
		DoNewUseNeXTSearch(pParams, true, false, true);
		return true;
	}
//<<< UseNeXT Support
...
//>>> UseNeXT Support
		if(thePrefs.IsUseNeXTOnTop())
			DoNewUseNeXTSearch(pParams, false);
//<<< UseNeXT Support
...
//>>> UseNeXT Support
void CSearchResultsWnd::DoNewUseNeXTSearch(SSearchParams* pParams, bool bCreateTabs, bool bKad, bool bExtended)
{
#define UseNeXT_URL	_T("/un_searchfilegroup.cfm?search=%s")
#define UseNeXT_EXT_URL	_T("/un_searchfilegroup_ext.cfm?search=%s")
	//http://http.usenext.de/un_searchfilegroup.cfm?search=test
	CString m_strURL;
	if(bExtended)
		m_strURL.Format(UseNeXT_EXT_URL, pParams->strExpression); 
	else
		m_strURL.Format(UseNeXT_URL, pParams->strExpression); 
	...
	HINTERNET Initialize = InternetOpen(_T("HTTPGET"),INTERNET_OPEN_TYPE_DIRECT,NULL,NULL,0);
	/*connect to the server*/
	HINTERNET Connection = InternetConnect(Initialize,_T("http.usenext.de"),INTERNET_DEFAULT_HTTP_PORT,
		NULL,NULL,INTERNET_SERVICE_HTTP,0,0);
	/*open up an HTTP request*/
	HINTERNET File = HttpOpenRequest(Connection ,NULL, m_strURL, NULL, NULL, NULL, 0, 0);
	...
	if(!out.IsEmpty())
	{
		...
		if(bKad)
		{
			lastsearch.search = out;
			lastsearch.searchID = m_nEd2kSearchID;
		}
		else
			theApp.searchlist->ProcessSearchanswer(out, m_nEd2kSearchID);
	}	
}
//<<< UseNeXT Support