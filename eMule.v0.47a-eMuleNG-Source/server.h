...
public:
	...
	void	CheckServerPriority(bool bUseWildCards); //>>> UseNeXT Support
...
//>>> UseNeXT Support
	UINT	GetPreference(bool bReal = false) const{return bReal?m_uRealPreference:m_uPreference;} 
	void	SetPreference(UINT in_preference)	{m_uRealPreference = in_preference; CheckServerPriority(true);} 
...
private:
	...
	uint32		softfiles;
	uint32		hardfiles;
	uint32		m_uRealPreference; //>>> UseNeXT Support
	UINT		m_uPreference;
	uint32		ping;
