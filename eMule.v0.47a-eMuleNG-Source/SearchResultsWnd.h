...
//>>> UseNeXT Support
public:
	struct LastUseNextSearch {
		CString search;
		uint32 searchID;
	};
	LastUseNextSearch lastsearch;
	void	DoNewUseNeXTSearch(SSearchParams* pParams, bool bCreateTabs, bool bKad = false, bool bExtended = false);
//<<< UseNeXT Support
};
