...
//>>> UseNeXT Support
	if(item1->IsUseNeXTFile() && item2->IsUseNeXTFile())
		; //TODO
	else if(item1->IsUseNeXTFile())
		return -1;
	else if(item2->IsUseNeXTFile())
		return 1;
//<<< UseNeXT Support
...
void CSearchListCtrl::DrawSourceParent(CDC *dc, int nColumn, LPRECT lpRect, /*const*/ CSearchFile* src)
{
	...
	if (lpRect->left < lpRect->right)
	{
		...
		switch (nColumn)
		{
			...
			case 2:			// avail
//>>> UseNeXT Support
				if(src->IsUseNeXTFile())
				{
					COLORREF oldcol = dc->SetTextColor(RGB(200, 0, 0));
					//if(thePrefs.IsUseNeXTDetected())
					buffer = _T("Highspeed-Download @ 6-MBit/s");
					//else
					//	buffer = _T("4MBit FullSpeed Trial");
					dc->DrawText(buffer, buffer.GetLength(), lpRect, DLC_DT_TEXT | DT_RIGHT);
...
//>>> UseNeXT Support
				if(src->IsUseNeXTFile())
				{
					COLORREF oldcol = dc->SetTextColor(RGB(0, 200, 0));
					buffer = _T("100%");
					dc->DrawText(buffer, buffer.GetLength(), lpRect, DLC_DT_TEXT | DT_RIGHT);
					dc->SetTextColor(oldcol);
					break;
