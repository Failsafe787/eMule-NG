...
//>>> UseNeXT Support
CSearchFile::CSearchFile(CString filename, LPCTSTR filetype, uint32 filesize, uint32 searchID, CString link)
{
	//uchar hash[16];
	//md4clr(hash);
	//md4cpy(m_abyFileHash, hash);
	for (int i = 0; i < 8; i++)
	{
		uint16 random = GetRandomUInt16();
		memcpy(&m_abyFileHash[i*2], &random, 2);
	}
	...
	taglist.Add(new CTag(FT_FILESIZE, filesize));
	taglist.Add(new CTag(FT_FILENAME, filename));
	taglist.Add(new CTag(FT_SOURCES, 500+rand()%1500));
	...
	m_nSearchID = searchID;
	m_eKnown = Unknown;
	m_bIsUseNeXTFile = true; 
}
//<<< UseNeXT Support
	...
	: CAbstractFile(copyfrom)
{
	...
	m_bIsUseNeXTFile = false; //>>> UseNeXT Support
}
