	...
	, m_ctrlTreeOptions(theApp.m_iDfltImageListColorFlags)
{
	...
	m_iMaxConnPerFive = 0;
	m_iMaxHalfOpen = 0;
	m_bUseNeXTOnTop = false; //>>> UseNeXT Support
	...
	m_htiUseNeXTOnTop = NULL; //>>> UseNeXT Support
...
void CPPgTweaks::DoDataExchange(CDataExchange* pDX)
{
	...
	if (!m_bInitializedTreeOpts)
	{
		...
		// Miscellaneous group
		//
		m_htiUseNeXTOnTop = m_ctrlTreeOptions.InsertCheckBox(_T("Usenet on top of search"), TVI_ROOT, m_bUseNeXTOnTop); //>>> UseNeXT Support
	...
	// Miscellaneous group
	//
	DDX_TreeCheck(pDX, IDC_EXT_OPTS, m_htiUseNeXTOnTop, m_bUseNeXTOnTop); //>>> UseNeXT Support
...
BOOL CPPgTweaks::OnInitDialog()
{
	...
	m_bUseNeXTOnTop = thePrefs.IsUseNeXTOnTop(); //>>> UseNeXT Support
...
BOOL CPPgTweaks::OnApply()
{
	...
	thePrefs.SetUseNeXTOnTop(m_bUseNeXTOnTop); //>>> UseNeXT Support
...
{	
	if(m_hWnd)
	{
		...
		if (m_htiUseNeXTOnTop) m_ctrlTreeOptions.SetItemText(m_htiUseNeXTOnTop, _T("Usenet on top of search")); //>>> UseNeXT Support
