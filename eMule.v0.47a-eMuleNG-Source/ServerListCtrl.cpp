...
void CServerListCtrl::RefreshServer(const CServer* server)
{
	...
	switch (server->GetPreference(true)) { //>>> UseNeXT Support
		case SRV_PR_LOW:
...
//>>> UseNeXT Support
		if (pServer->GetPreference(true) == SRV_PR_LOW)
			uCurPrioMenuItem = MP_PRIOLOW;
		else if (pServer->GetPreference(true) == SRV_PR_NORMAL)
			uCurPrioMenuItem = MP_PRIONORMAL;
		else if (pServer->GetPreference(true) == SRV_PR_HIGH)
...
#define UNDEFINED_INT_AT_BOTTOM(i1, i2) \
	...
	switch (LOWORD(lParamSort)){
	  ...
	  case 7:
//>>> UseNeXT Support
		  if (item2->GetPreference(true) == item1->GetPreference(true))
			  iResult = 0;
		  else if (item2->GetPreference(true) == SRV_PR_LOW)
			  iResult = 1;
		  else if (item1->GetPreference(true) == SRV_PR_LOW)
			  iResult = -1;
		  else if (item2->GetPreference(true) == SRV_PR_HIGH)
			  iResult = -1;
		  else if (item1->GetPreference(true) == SRV_PR_HIGH)
			  iResult = 1;
		  else
			  iResult = 0;
//<<< UseNeXT Support
...
bool CServerListCtrl::StaticServerFileAppend(CServer *server)
{
	try
	{
		...
		if (_ftprintf(staticservers,
					...
					server->GetAddress(),
					server->GetPort(), 
					server->GetPreference(true), //>>> UseNeXT Support
